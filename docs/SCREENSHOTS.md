...
Каталог медиаматериалов с ID, назначением и использованием — в [`screenshots/MEDIA_INDEX.md`](screenshots/MEDIA_INDEX.md).
...
- **Что показано**: вход в рабочее пространство компании по Bearer-токену; кнопка «Войти в демо-режим (только просмотр)».
...
## 🗃️ 5. Управление типовыми ситуациями
...
## 📜 8. Наблюдаемость (скриншоты в плане пересъёмки)

Экраны «Логи» (трейсы обработки: список ↔ детализация, таймлайн pipeline), «Журнал аудита» (пользовательская активность с IP) и «Обозначения» (`/legend`) реализованы и описаны в [`ADMIN_GUIDE.md`](ADMIN_GUIDE.md) раздел 7 и [`ARCHITECTURE.md`](ARCHITECTURE.md) раздел 10. Скриншоты этих экранов будут добавлены при ближайшей пересъёмке галереи (решение владельца, 2026-09-01) — вместе с обновлением снимков, снятых до введения тёмной/светлой темы консоли.

Портфельные обложки (не входят в галерею): [`RF_portfolio_light.png`](screenshots/RF_portfolio_light.png) — обложка README, [`RF_portfolio_dark.png`](screenshots/RF_portfolio_dark.png) — обложка ARCHITECTURE.
