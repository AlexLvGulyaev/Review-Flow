...
flowchart TB
    subgraph "Пользователи"
        C["Клиент"]
        OP["Оператор компании"]
        ADM["Администратор / руководитель"]
    end

    RF["Review Flow<br/>ИИ-ассистент для работы с обращениями клиентов<br/>(Controlled Hybrid)"]

    subgraph "Внешние системы"
        LLM["LLM-провайдер<br/>OpenAI / GigaChat"]
    end

    C -->|"HTTP — оставить обращение, проверить статус"| RF
    OP -->|"HTTP — рабочее место оператора"| RF
    ADM -->|"HTTP — консоли, отчёты, настройки"| RF
    RF -->|"Chat Completions — генерация черновика ответа"| LLM
```

- **Клиент** — публичный портал без регистрации: оставляет обращение, проверяет статус
  по номеру и email.
- **Оператор** — контур компании по Bearer-токену: очередь обращений, проверка
  Controlled Hybrid, публикация ответа, эскалация.
- **Администратор / руководитель** — база знаний (Response Cases), кандидаты, отчёты,
  системные настройки, наблюдаемость.
- **LLM-провайдер** — OpenAI / GigaChat через единую абстракцию; генерирует адаптированный
  текст в рамках policy (bounded generation).

---

## 📦 2. Container Diagram (C4 Level 2)

Внутреннее устройство: контейнеры, их ответственности и связи.
...
### 3.3 Контур компании — администратор
...
### 4.1 Примеры ключевых API (не полный список)
...
## 🗄️ 5. PostgreSQL и миграции (as‑is)

### 5.1 Инициализация БД в Docker Compose
...
### 5.2 Применение миграций backend (as‑is)
...
## 🧠 6. Controlled Hybrid pipeline (as‑is + норматив)
...
## 🌱 8. Lifecycle новой типовой ситуации (learning loop)
...
## 🎭 9. Роли retrieval / LLM / оператора / администратора
...
## 🛣️ 12. Roadmap / future work (строго как планы)
