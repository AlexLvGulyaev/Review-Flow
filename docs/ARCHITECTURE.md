...
Нормативный источник решений: [Архитектурные и продуктовые решения (SOT)](../Архитектурные_и_продуктовые_решения_проекта_SOT_v4.md).
...
**Ключевой инвариант (Controlled Hybrid):** клиент **не видит** внутренних сущностей `Response Case`, confidence и результаты retrieval (см. [операционную модель CH](architecture/controlled_hybrid_operational_model.md), раздел о C3).
...
Нормативное описание: [операционная модель Controlled Hybrid](architecture/controlled_hybrid_operational_model.md).  
Forensics после интеграции CH: [CH pipeline forensics](architecture/ch_pipeline_forensics_after_ch_integration.md).
...
Детальная семантика статусов для клиентского UX описана в [операционной модели CH](architecture/controlled_hybrid_operational_model.md) (раздел о C3: клиент не должен видеть внутренние статусы/сущности).
...
- UI контуры реализованы как одно приложение с role‑переключением (см. [план разделения UI‑контуров](architecture/ui_contour_separation_plan.md)).
...
- визуальное разделение client site и company workspace ([план разделения UI‑контуров](architecture/ui_contour_separation_plan.md));
- усиление контракта “operational console” (AF‑alignment) для рабочих мест ([семантическое выравнивание](architecture/review_flow_af_operational_semantics_alignment.md));
