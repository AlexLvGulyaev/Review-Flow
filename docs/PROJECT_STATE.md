...
**Консоль и качество CH (2026-09-01):** тёмная/светлая тема консоли (useTheme + токен-карта `rf-tokens.css`), единый эмодзи-контракт чипов (`chipContract` + `OpChipFor` — составные подсказки «Семья: Значение»), экран «Обозначения» (`/legend`), вход в консоль по токену как в AIC; CH confidence — gap-based banding (`score_floor`/`match_gap`/`confidence_gap_high`, миграция `017`) с перекалибровкой порогов; системный промпт генерации переписан на русский (v2) — единственный источник истины — активная версия в реестре `review_response_generation` (бейдж версии в настройках, раздел 11 в CONTROLLED_HYBRID.md); UX-фиксы E2E-прогона (навигация стрелками, пагинация журналов, autofill).
...
4. **FL.ru #5507055** — ИИ агент для заведения (анализ звонков). Review Flow закрывает анализ текста и отчётность.
5. **FL.ru #5508319** — Голосовой робот для приёма звонков. Review Flow закрывает логику квалификации.
...
Проект принят в портфель как ключевой актив направления "Классификация и обработка обращений".
...
| 2026-09-01 | Консоль 2.0: тема, эмодзи-контракт, gap-banding, промпт v2 | Тёмная/светлая тема + экран «Обозначения» (`/legend`), эмодзи-контракт чипов, вход по токену; CH gap-based banding (017) + перекалибровка порогов; русский системный промпт v2 (реестр — единственный SoT, бейдж версии, раздел 11 CONTROLLED_HYBRID); UX-фиксы E2E (стрелки, пагинация, autofill); аудит документов кейса, обложки README/ARCHITECTURE из портфельных скриншотов |
| 2026-09-03 | Снят тезис CRM-интеграций (решение владельца) | Из Market Validation убран заказ #5507454 (Kommo); из рисков/матрицы/Next Steps убраны пункты CRM/n8n-интеграций; §18.5 API_CONTRACT (Kommo/Bitrix24) удалена — проект позиционируется без CRM-слоя |
| 2026-09-03 | Обязательный технический долг закрыт (фиксация) | Решение владельца 03.09: обязательный технический долг проекта закрыт; оставшиеся пункты обязательным долгом не являются и ведутся как SHOULD/COULD-роадмап — пересъёмка галереи скриншотов (новые экраны Логи/Аудит/Обозначения + снимки до тёмной темы) — по готовности владельца |
...
**Статус:** Актуален — паспорт состояния проекта
