...
API использует **заголовок `X-Role`** для выбора роли. Параметры входа по логину/паролю в текущей версии не предусмотрены: роль определяется исключительно значением заголовка.

| Роль | Значение заголовка `X-Role` | Доступ |
|---|---|---|
| Клиент (публичный доступ) | отсутствует / любое неизвестное значение | публичные эндпоинты создания и проверки статуса обращений |
| Оператор | `operator` | операторская очередь и модерация |
| Администратор | `administrator` | все эндпоинты, включая администрирование, аналитику, настройки |

> **Важно:** пустой или нераспознанный `X-Role` автоматически приравнивается к роли `client`. Оператор и администратор должны явно передавать свой заголовок.
...
| `403 Forbidden` | Недостаточно прав для роли | `{"detail": "Access denied for role '...'"}` |
