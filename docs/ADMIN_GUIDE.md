...
Редактирование активного системного промпта `review_response_generation` (как LLM адаптирует утверждённый текст). **«Сохранить»** создаёт новую версию в реестре промптов и активирует её — активная версия показывается бейджем `vN` в заголовке панели (смена версий фиксируется в журнале аудита). Активная версия реестра — единственный источник промпта генерации; встроенный текст в коде используется только если реестр пуст. Семантика и текущий текст — [🧠 `CONTROLLED_HYBRID.md`](CONTROLLED_HYBRID.md), раздел 11.
...
Группа «Наблюдаемость» бокового меню содержит два раздельных экрана.

### 7.1. «Логи» — трейсы обработки обращений

Служит вопросу «**как система обработала обращение**: запрос пользователя → ответ системы?».

Журнал построен по обращениям: **одна строка списка = одно обращение** (на входе — обращение пользователя, на выходе — статус и ответ). Логирует сам пайплайн обработки; открытия экранов и чтения консоли в журнал не попадают (см. 7.2 — там действия персонала).

- Слева — список обращений: время поступления, статус значком (🟢/❌/⏳), номер обращения, модель и latency pipeline (простым текстом, без подсветки).
- Фильтры: окно времени (`24h`/`7d`/`30d`/`все`), статус (🟢 обработано / ❌ ошибка / ⏳ в обработке), поиск по номеру или ID обращения — отдельной строкой под фильтрами. Пагинация — по 7 записей на странице. Список пролистывается стрелками ↑/↓ (с переходом через страницы).
- Справа — развёрнутый трейс «ДЕТАЛИЗАЦИЯ ЗАПРОСА» (панели в порядке как у референса): «Параметры запроса» / «Параметры исполнения» (модерация, публикация), «Цепочка обработки» одной строкой, затем **«Запрос пользователя»** — полный текст обращения и **«Ответ системы»** — финальный ответ (до модерации — черновик), «Ошибка» (при наличии), и последней — **«Таймлайн pipeline»**: карточки этапов (время → статус → этап → накопленный offset → «+latency»), у каждого этапа раскрывается «JSON payload»; замыкает панель «Технический снимок (JSON)». Тексты читаются из обращения и ответа по ссылке, а не дублируются в логе.
- **Экспорт CSV** выгружает журнал построчно по обращениям с текущими фильтрами: колонки «обращение (вход)», «ответ системы (выход)», «этапы обработки» и др. (файл открывается в Excel).

### 7.2. «Журнал аудита» — пользовательская активность

Журнал фиксирует, **кто и что делал** в системе, в трёх контурах:

| Контур | События |
|---|---|
| Администратор 🛡️ | мутации НСИ и настроек, работа с типовыми ситуациями, версии промптов |
| Оператор 🎧 | модерация ответов, подтверждение/смена ситуации, эскалации |
| Клиент 👤 | отправка обращения, проверка статуса обращения (пресейловый сигнал — кто заинтересовался системой) |
| Демо 🎭 | вход в read-only демо-сессию |

- Каждая запись: время, значок роли, действие, тип ресурса, IP-адрес. Роль берётся из токена доступа; для клиентского контура и демо — фиксированные роли `Клиент`/`Демо`. Список пролистывается стрелками ↑/↓ (с переходом через страницы).
- Фильтры: окно времени, роль, тип ресурса, действие. Справа — детализация: «ДЕТАЛИЗАЦИЯ СОБЫТИЯ» с кодом акции, «Параметры акции» (ID/тип акции, ID/тип ресурса), «Параметры пользователя» (ID/имя пользователя, IP-адрес, дата события), «Детали / metadata» и снимок JSON.
- **Экспорт CSV** — с текущими фильтрами.

Значки ролей описаны на экране «Обозначения» (раздел «Участник журнала аудита»).

---

## 🛠️ 8. Типовые проблемы
